<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Connexion - Inscription</title>
    <link rel="stylesheet" href="style.css">
</head>
<body>

    <?php
    
    include('config.php');

    ?>

    <img id="logo" src="Images/Logo.png" alt="Logo de Work4Green">

    <img id="fond" src="Images/FondWork4Green.png" alt="Image de fond">
    
    <div class="boutons">

        <p>Plus de deux entreprises font une différence grâce à notre application.</p>

        <a id="registration" href="registration.php">S'inscrire</a>

        <a id="connection" href="connection.php">Se connecter</a>
        
        <a id="admininscription" href="admininscription.php">S'inscrire en tant que chef·fe d'entreprise</a>

    </div>

</body>
</html>
<?php

